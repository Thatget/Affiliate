...
class ApplyRulesCronEvery
{
     ...
     */
    public function execute($idRule = null)
    {
            ...
            if (sizeof($productCollection) > 0) {
                ...
                if (sizeof($catalogruleIds) > 0) {
                    ...
                    if ($catalogrules->count() > 0) {
                                    ...
                                    if ($catalogrule->getConditions()->validate($product)) {
                                        ...
                                        if ($checkInserts->getSize() == 0) {
                                            ...
                                            foreach ($checkInserts as $checkInsert) {
                                                if ($data['reward_point'] > 0) {
                                                    /*  udpate */
                                                    $case = $connection->quoteInto('?', $checkInsert->getId());
                                                    $this->_CONDITION['convert_point'][$case] = $data['reward_point'];
                                                    $this->_ITEMS[] = $checkInsert->getId();
                                                } else if ($data['reward_point'] == 0) {
                                                    $this->_ITEMS_DELETE[] = $checkInsert->getId();
                                                }
                                            }
                                        }
                                    } else {
                        ...
                        if($this->_CONDITION['convert_point']){
                            $values = array(
                                'reward_point' => $connection->getCaseSql('id', $this->_CONDITION['convert_point'],0),
                            );
                        }else{
                            $values = array();
                        }
                        $insert = $this->_INSERTS;
                        $delete = $this->_ITEMS_DELETE;
                        $where = array('id IN (?)' => $this->_ITEMS);
                        $query = array(
                            'delete' => $delete,
                            'insert' => $insert,
                            'values' => $values,
                            'condition' => $where,
                            'table' => $resource->getTableName('mw_reward_product_point')
                        );
                        $this->_productpointFactory->create()->updateData($query);
